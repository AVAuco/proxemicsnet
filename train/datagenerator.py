...
from train.load_dataset_train_test import feature_extractor_ViT
...
class DataGenerator(keras.utils.Sequence):
	...
	def __init__(self,  list_IDs, allSamples, batch_size=32, shuffle=True, augmentation=True, isTest=False,zipPath='', posezipPath='', onlyPairRGB=False,onlyPairPose=False,  typeImg='', isTransformer=False ):
		'Initialization'
	...
	def __data_generation(self, list_IDs_temp):
		...
		for i in range(0, len(list_IDs_temp)):
			...
			#if p0 is None or p1 is None or pair is None : 
			#	continue
		...
		if self.isTransformer==False:
			...
			if self.onlyPairRGB==False:
				X.append(feature_extractor_ViT(p0_fb))
				X.append(feature_extractor_ViT(p1_fb))
			X.append(feature_extractor_ViT(pair_fb))
